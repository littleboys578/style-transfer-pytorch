...
def download_file(cv_path,target):
	opener=urllib.request.build_opener()
	opener.addheaders=[('User-Agent','Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/36.0.1941.0 Safari/537.36')]
	urllib.request.install_opener(opener)
	url = cv_path
	urllib.request.urlretrieve(url, target+url.split("/")[-1])

@app.route('/tranforImage',methods=['GET','POST'])
def save_images():
	fina_res={}
	data = {}
	...
	content_weight = data.get("threshold")
	try:
		download_file(style,"./styles/")
		download_file(content,"./content/")
		style_name = style.split("/")[-1]
		content_name = content.split("/")[-1]
		args.style = "./styles/"+style_name
		args.content = "./content/"+content_name
...
def generate_images():
	...
	print('Loading model...')
	st = StyleTransfer(devices=['cpu'], pooling=args.pooling)
	...
	# target_path = project_folder + 'outputs_data/' + imagename + '.png'
	# cv2.imwrite(target_path, img1, [int(cv2.IMWRITE_JPEG_QUALITY), 90])
	# f = open(target_path, 'rb')
	# resp = Response(f.read(), mimetype="image/png")
	imagename = hashlib.md5(str(random.random()).encode()).hexdigest()
	path = './outputs_data/'+ imagename + '.png'
	if output_image is not None:
		save_image(path, output_image)
	return path

if __name__ == '__main__':
	# args.style = "./styles/"+"starry_night.jpg"
	# args.content = "./content/"+"golden_gate.jpg"
	# generate_images()
